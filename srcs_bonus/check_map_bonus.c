...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/01 14:17:57 by tcharpen          #+#    #+#             */
/*   Updated: 2022/05/01 14:17:57 by tcharpen         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_elements(t_map *map, int i_map, int j_map)
{
	if (map->map[i_map][j_map] == 'E')
		...
		map->nbr_p++;
	else if (map->map[i_map][j_map] == 'V' ||
		map->map[i_map][j_map] == 'H')
		map->nbr_m++;
...
static void	map_contains_elements(t_map *map)
{
	...
	while (map->map[++i_map])
	{
		...
		{
			if (!check_elements(map, i_map, j_map))
